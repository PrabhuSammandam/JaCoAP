...
public:
	...
	void            print( void );        //prints the contents of the linked list
...
private:
	...
	bool            isEmpty( void );      //utility functions used to see if the list contains no elements
...
bool List<T>::isEmpty( void ) {
	if ( startPtr == nullptr && endPtr == nullptr ) {
		...
		//if the start pointer and end pointer are NULL then the list is empty
		return true;
	}
	else {
		return false;
...
void List<T>::insertNewNode( T dataIn )   //general funtionn to insert new node the proper order in the list
{
	if ( isEmpty( ) ) {
		...
		if ( dataIn < startPtr->data ) {
			...
			//the new node is being inserted in order but not at the beginning or end
			ListNode<T>*    currentPtr = startPtr;
			ListNode<T>*    newPtr = new ListNode<T>( dataIn );  //creates new node
